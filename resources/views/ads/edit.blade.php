                        ...
                        <form action="{{ route('ads.update',$ad->id) }}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-select select2 type_select form-control" name="type" id="type"  aria-label="Default select">
                                    ...
                                    <option value="{{ $ad->type }}" {{ $ad->type == 1 ? "selected":"" }}>Top</option>
                                    <option value="{{ $ad->type }}" {{ $ad->type == 2 ? "selected":"" }}>Middle</option>
                                    <option value="{{ $ad->type }}" {{ $ad->type == 3 ? "selected":"" }}>Bottom</option>
                                </select>
                                @error('type')
                                <small class="invalid-feedback font-weight-bold" role="alert">
                                    {{ $message }}
